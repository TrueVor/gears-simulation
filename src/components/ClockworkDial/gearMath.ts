...
export function activeIndexToAngle(activeIndex: number, dotCount: number, startAngle: number): number {
...
export function indexToValue(index: number, min: number, max: number, dotCount: number): number {
  if (dotCount <= 1) return min;
  return Math.round(min + (index * (max - min)) / (dotCount - 1));
}

export function valueToIndex(value: number, min: number, max: number, dotCount: number): number {
  if (dotCount <= 1 || max === min) return 0;
  const clamped = Math.min(max, Math.max(min, value));
  return wrapIndex(Math.round(((clamped - min) / (max - min)) * (dotCount - 1)), dotCount);
}
...
export function createMeshedGears(gearSpeed: number): GearConfig[] {
  ...
  return [
    { id: 'center', cx: 105, cy: 126, radius: 28, teeth: centerTeeth, angularSpeed: centerSpeed, fill: 'url(#steelGradient)', stroke: '#111827' },
    { id: 'right', cx: 160, cy: 126, radius: 56, teeth: rightTeeth, angularSpeed: calculateDrivenSpeed(centerSpeed, centerTeeth, rightTeeth, 'external'), initialRotation: 7.5, fill: 'url(#bronzeGearGradient)', stroke: '#241407' },
  ];
}
