...
export function ClockworkDial({
  ...
  const [renderedAngle, setRenderedAngle] = useState(
    activeIndexToAngle(activeIndex, dotCount, startAngle),
  );
  ...
  const runtimeGears: GearRuntimeConfig[] = useMemo(
    () =>
      gears.map((gear, index) => ({
        ...gear,
        angularSpeed: gear.angularSpeed ?? 0,
        ref: index === 0 ? centerGearRef : rightGearRef,
      })),
    [gears],
  );
  ...
  const selectIndex = (nextIndex: number) => {
    ...
    setRenderedAngle(
      (previous) => previous + shortestAngleDelta(previous, targetAngle),
    );
  ...
  const style = {
    "--dial-size": typeof size === "number" ? `${size}px` : size,
  } as CSSProperties;
      ...
      >
        <defs>
          <radialGradient id="plateGradient">
            <stop offset="0%" stopColor="#30261f" />
            <stop offset="100%" stopColor="#110f12" />
          </radialGradient>
          <linearGradient id="bronzeGradient" x1="0" x2="1">
            <stop stopColor="#5d3216" />
            <stop offset=".5" stopColor="#c78635" />
            <stop offset="1" stopColor="#321808" />
          </linearGradient>
          <linearGradient id="bronzeGearGradient" x1="0" x2="1">
            <stop stopColor="#6a3a18" />
            <stop offset=".55" stopColor="#d0983f" />
            <stop offset="1" stopColor="#3a1f0e" />
          </linearGradient>
          ...
          <radialGradient id="axleGradient">
            <stop offset="0%" stopColor="#fff6b5" />
            <stop offset="65%" stopColor="#d99921" />
            <stop offset="100%" stopColor="#7c3f0b" />
          </radialGradient>
          <radialGradient id="activeDotGradient">
            <stop offset="0%" stopColor="#fff8a8" />
            <stop offset="100%" stopColor="#f2b300" />
          </radialGradient>
          <linearGradient id="pointerGradient" x1="0" x2="1">
            <stop stopColor="#fff4cf" />
            <stop offset="1" stopColor="#c15b3b" />
          </linearGradient>
          <linearGradient id="badgeGradient" y1="0" y2="1">
            <stop stopColor="#fff4d6" />
            <stop offset="1" stopColor="#d9b68a" />
          </linearGradient>
        </defs>
        <Gear key={gears[1].id} gear={gears[1]} ref={rightGearRef} />
        <circle
          cx="120"
          cy="126"
          r="109"
        ...
        <circle
          ...
          strokeWidth="8"
        />
        <Gear key={gears[0].id} gear={gears[0]} ref={centerGearRef} />
        <IndicatorRing
          dotCount={dotCount}
